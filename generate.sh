#!/usr/bin/env bash
#
# generate.sh — Rebuild index.html from all project data.
#
# Scans data/*/history.json, discovers all projects, and generates
# a self-contained dashboard page with charts and tables.
#
# Usage:  ./generate.sh
# Run from the repo root (next to the data/ directory).
...
PROJECTS=()
for dir in data/*/; do
  [ -f "${dir}history.json" ] && PROJECTS+=("$(basename "$dir")")
done

if [ ${#PROJECTS[@]} -eq 0 ]; then
  echo "No projects found in data/*/history.json"
  exit 0
fi
...
# ── Build the page ───────────────────────────────────────────────────
# We write the HTML in three chunks: head → project options → tail.
# This avoids any sed/awk escaping headaches with the JS code.

PROJECT_OPTIONS=""
for p in "${PROJECTS[@]}"; do
  PROJECT_OPTIONS="${PROJECT_OPTIONS}    <option value=\"${p}\">${p}</option>
"
done
...
# inject discovered project <option> tags
echo -n "$PROJECT_OPTIONS" >> index.html
...
echo "Done — index.html generated with ${#PROJECTS[@]} project(s)"
